  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // En caso de fallo (por ejemplo, en servidores de sólo lectura de Vercel)
        ...
        // Almacenar localmente en localStorage para que se visualice en este navegador
          ...
          <div className="flex flex-col gap-3">
            ...
            <ul className="flex flex-col gap-2 mt-2">
              {characteristics.map((char, index) => (
                <li key={index} className="flex justify-between items-center gap-4 bg-zinc-50/50 dark:bg-zinc-900/10 border border-zinc-100 dark:border-zinc-900/50 px-4 py-2.5 rounded-xl text-xs sm:text-sm text-zinc-650 text-zinc-700 dark:text-zinc-300">
      ...
      {showFallbackModal && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-zinc-950 border border-zinc-150 dark:border-zinc-900 rounded-3xl p-6 sm:p-8 max-w-2xl w-full shadow-2xl flex flex-col gap-6 animate-slide-up">
            ...
            >
              Entendido y Volver al Catálogo
            </button>
          </div>
        </div>
      )}
